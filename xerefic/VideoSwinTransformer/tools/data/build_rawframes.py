...
if __name__ == '__main__':
    ...
    for classname in classes:
        vid_dict[classname] = []
    for vid in vid_list:
        vid_dict[vid.split('/')[0]].append(vid.split('/')[1])

    new_vid_list = []
    for classname in classes:
        try: 
            sampler = random.sample(range(len(vid_dict[classname])), 100)
        except:
            sampler = range(len(vid_dict[classname]))
        vid_dict[classname] = [vid_dict[classname][i] for i in sampler]

        for vid in vid_dict[classname]:
            new_vid_list.append(os.path.join(classname, vid))
