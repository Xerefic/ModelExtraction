...
def parse_kinetics_annotations(input_csv, ignore_is_cc=False):
    ...
    if 'youtube_id' in df.columns:
        ...
        new = pd.DataFrame(columns = ['video-id', 'start-time', 'end-time', 'label-name'])

        classes = {}
        for row in range(len(df)):
            label = df.iloc[row]['label-name']
            if label in classes.keys():
                classes[label] += 1
                if classes[label] <= 0.1 * len(df['label-name'] == label):
                    new = new.append(df.iloc[row], ignore_index = True)
                    print(len(new), end='\r')
            else:
                classes[label] = 1
    return new 
