...
from models import dataset_dla
...
class DLA(nn.Module):
    def __init__(self, config, levels, channels, num_classes=1000,
    ...
    def load_pretrained_model(self, data_name, name):
        assert data_name in dataset_dla.__dict__, \
            'No pretrained model for {}'.format(data_name)
        data = dataset_dla.__dict__[data_name]
        fc = self.fc
...
def dla34(config, pretrained=None, **kwargs):  # DLA-34
    model = DLA(config, [1, 1, 1, 2, 2, 1],
...
def dla60(config, pretrained=None, **kwargs):  # DLA-60
    Bottleneck.expansion = 2
    model = DLA(config, [1, 1, 1, 2, 3, 1],
...
def dla102(config, pretrained=None, **kwargs):  # DLA-102
    Bottleneck.expansion = 2
    model = DLA(config, [1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024],
...
def dla169(config, pretrained=None, **kwargs):  # DLA-169
    Bottleneck.expansion = 2
    model = DLA(config, [1, 1, 2, 3, 5, 1], [16, 32, 128, 256, 512, 1024],
                block=Bottleneck, residual_root=True, **kwargs)
    if pretrained is not None:
...
def make_model(config):
    version = str(config['model']['backbone']['version'])
    fn = 'dla' + version
    pretrained = config['model']['backbone']['pretrained']
    
    return globals()[fn](config, pretrained)


if __name__=='__main__':
    import hiddenlayer as hl
    transforms = [ hl.transforms.Prune('Constant') ]
    model = dla34(None).to('cuda')
    inp = torch.rand(1,3,224,224).to('cuda')
    out = model(inp)
    print(out.size())
    #graph = hl.build_graph(model, inp, transforms = [])
