...
def inference_pytorch(args, cfg, distributed, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            bs = data['imgs'].shape[0]
            data['imgs'] = data['imgs'].view(bs, -1, 3, 8, 224, 224).to(device)
            result = torch.empty((bs, 0, 600))
        ...
        for _ in range(batch_size):
            prog_bar.update()
        with open('/home/ubuntu/ModelExtraction/new/Video-Swin-Transformer/checkpoints/movinet/logits.pkl', 'wb') as f:
            pkl.dump(results, f)
    return results
    # return outputs
...
def main():
    ...
    # for i in range(32):
    #     # print(next(iter(data_loader))['imgs'].shape)
    #     data = next(iter(data_loader))['imgs']
    #     # print(data[0, 0, :, i].shape)
    #     # quit()
    #     save_image(data[0, 0, :, i], '/home/ubuntu/ModelExtraction/new/Video-Swin-Transformer/data/mit/frames1/' + f"img{i+1}.png") 
    #     save_image(data[0, 1, :, i], '/home/ubuntu/ModelExtraction/new/Video-Swin-Transformer/data/mit/frames2/' + f"img{i+1}.png") 
